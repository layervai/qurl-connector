...
func reconcileConnectorResourceDeletion(ctx context.Context, client *qurl.Client, slug, crid string, alreadyVerified bool) (deletedNow bool, err error) {
	if client == nil {
		return false, errors.New("qURL resource client is nil")
	}
	if !alreadyVerified {
		resource, lookupErr := client.GetConnectorResource(ctx, crid)
		if connectorResourceReadProvesDeletion(lookupErr) {
			return false, nil
		}
		if lookupErr != nil {
			return false, fmt.Errorf("inspect qURL resource %s before deletion: %w", crid, lookupErr)
		}
		if err := validateConnectorDeletionTarget(resource, slug, crid); err != nil {
			return false, err
		}
	}

	deleteErr := client.DeleteConnectorResource(ctx, crid)
	if deleteErr == nil {
		return true, nil
	}
	if errors.Is(deleteErr, qurl.ErrConnectorResourceNotFound) {
		return false, fmt.Errorf("delete qURL resource %s returned not found; exact retry identity is preserved: %w", crid, deleteErr)
	}
	...
	if lookupErr != nil {
		return false, errors.Join(
			fmt.Errorf("delete qURL resource %s: %w", crid, deleteErr),
			fmt.Errorf("reconcile qURL resource %s after uncertain deletion: %w", crid, lookupErr),
		)
	}
	if err := validateConnectorDeletionTarget(resource, slug, crid); err != nil {
		return false, errors.Join(fmt.Errorf("delete qURL resource %s: %w", crid, deleteErr), err)
	}
	return false, fmt.Errorf("delete qURL resource %s remains unresolved and the exact resource is still active; retry state is preserved: %w", crid, deleteErr)
}
...
func validateConnectorDeletionTarget(resource *qurl.ConnectorResource, slug, crid string) error {
	if resource == nil {
		return errors.New("qURL service returned no Connector resource while confirming deletion target")
	}
	if resource.CRID != crid {
		return fmt.Errorf("Connector id %q resolved crid %q while confirming deletion target, want %q", slug, resource.CRID, crid)
	}
	if resource.Slug != slug {
		return fmt.Errorf("crid %q belongs to Connector id %q, not %q; refusing deletion", crid, resource.Slug, slug)
	}
	return nil
}
