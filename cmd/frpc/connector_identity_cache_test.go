...
func TestConnectorIdentityCacheStrictSchema(t *testing.T) {
	tests := map[string]struct {
		...
		"missing crid":                {`{"version":3,"agent_id":"agent-remove","identities":[{"id":"web","resource_public_key":"` + testPublicResourceID + `","connector_routing_id":"` + testConnectorRoutingID + `","knock_resource_id":"cell-resource"}],"pending_requests":[]}`, "crid is required"},
		"mismatched crid":             {`{"version":3,"agent_id":"agent-remove","identities":[{"id":"web","resource_public_key":"` + testPublicResourceID + `","crid":"` + testCRIDForKey(testPublicResourceID2) + `","connector_routing_id":"` + testConnectorRoutingID + `","knock_resource_id":"cell-resource"}],"pending_requests":[]}`, "crid must cryptographically match"},
		"crid aliases knock id":       {`{"version":3,"agent_id":"agent-remove","identities":[{"id":"web","resource_public_key":"` + testPublicResourceID + `","crid":"` + testCRIDForKey(testPublicResourceID) + `","connector_routing_id":"` + testConnectorRoutingID + `","knock_resource_id":"` + testCRIDForKey(testPublicResourceID) + `"}],"pending_requests":[]}`, "must be distinct"},
		"public key as expected crid": {`{"version":3,"agent_id":"agent-remove","identities":[],"pending_requests":[{"id":"web","request_nonce":"` + testConnectorRequestNonce + `","expected_crid":"` + testPublicResourceID + `"}]}`, "expected_crid"},
		"null crid":                   {`{"version":3,"agent_id":"agent-remove","identities":[{"id":"web","resource_public_key":"` + testPublicResourceID + `","connector_routing_id":"` + testConnectorRoutingID + `","knock_resource_id":"cell-resource","crid":null}],"pending_requests":[]}`, "crid must not be null"},
		...
		"null pending expected id":    {`{"version":3,"agent_id":"agent-remove","identities":[],"pending_requests":[{"id":"web","request_nonce":"` + testConnectorRequestNonce + `","expected_crid":null}]}`, "expected_crid must not be null"},
